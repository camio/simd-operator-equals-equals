...
[^1]: Formally, we can say μ⟦`std::simd<` *T*`, std::simd::abi::fixed_size<` *N*`> `⟧ = μ⟦*T*⟧ × μ⟦*T*⟧ ⋯ μ⟦*T*⟧ (μ⟦*N*⟧ times)
...
Here, a user might make the following modification:
...
for every structure that embeds a `std::simd`.
...
The following standard library types, like `std::simd`, represent sequences of
values: `std::bitset`, `std::array`, `std::vector`, `std::list`, and
`std::valarray`. All of these, have regular `==` operators with the exception of
`std::valarray`, which is rarely used due to its poor design.[^3]
...
Lane for a never-ending supply of friendly opposition.
